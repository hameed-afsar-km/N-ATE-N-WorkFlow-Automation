...
function NodeCard({ def }: { def: NodeTypeDef }) {
  ...
  return (
    <div
      ...
      className="group flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-grab active:cursor-grabbing transition-all duration-200"
      style={{ border: '1px solid transparent' }}
      onMouseEnter={(e) => { const el = e.currentTarget as HTMLDivElement; el.style.background = `${def.color}11`; el.style.borderColor = `${def.color}33`; }}
    ...
    >
      <div className="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: `${def.color}22`, color: def.color }}>
        <NodeIcon name={def.icon} size={15} />
      </div>
      <div className="flex-1 min-w-0">
        <div className="text-xs font-semibold text-white truncate">{def.label}</div>
        <div className="text-[10px] truncate" style={{ color: '#4a4e78' }}>{def.description}</div>
      </div>
      <Plus size={13} className="opacity-0 group-hover:opacity-60 transition-opacity flex-shrink-0" style={{ color: def.color }} />
    </div>
  );
}
...
function CategorySection({ color, label, nodes }: { categoryId: NodeCategory; color: string; label: string; nodes: NodeTypeDef[] }) {
  ...
  return (
    <div className="mb-1">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg transition-colors hover:bg-white/5">
        <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: color }} />
        <span className="text-xs font-bold uppercase tracking-widest flex-1 text-left" style={{ color }}>{label}</span>
        <span className="text-[10px] mr-1" style={{ color: '#4a4e78' }}>{nodes.length}</span>
        {open ? <ChevronDown size={13} style={{ color: '#4a4e78' }} /> : <ChevronRight size={13} style={{ color: '#4a4e78' }} />}
      </button>
      {open && <div className="pl-1">{nodes.map(def => <NodeCard key={def.type} def={def} />)}</div>}
    </div>
  );
}
  ...
  return (
    <>
      ...
      <aside className="flex-shrink-0 flex flex-col h-full transition-all duration-300 overflow-hidden"
        style={{ width: isSidebarOpen ? 280 : 0, background: 'rgba(10,11,20,0.98)', borderRight: '1px solid rgba(37,40,72,0.8)' }}>
        {isSidebarOpen && (
          <>
            <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid rgba(37,40,72,0.8)' }}>
              <span className="text-xs font-bold uppercase tracking-widest" style={{ color: '#4a4e78' }}>Panel</span>
              <button onClick={toggleSidebar} className="p-1.5 rounded-lg hover:bg-white/5" style={{ color: '#4a4e78' }}><PanelLeftClose size={15} /></button>
            </div>
            <div className="flex gap-1 p-2" style={{ borderBottom: '1px solid rgba(37,40,72,0.8)' }}>
              {TABS.map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-[11px] font-semibold transition-all"
                  style={{ background: activeTab === tab.id ? 'rgba(108,99,255,0.15)' : 'transparent', color: activeTab === tab.id ? '#6c63ff' : '#4a4e78', border: activeTab === tab.id ? '1px solid rgba(108,99,255,0.3)' : '1px solid transparent' }}>
                  {tab.icon} {tab.label}
                </button>
              ))}
            </div>
                ...
                <div>
                  <div className="p-2">
                    <div className="relative">
                      <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: '#4a4e78' }} />
                  ...
                  <div className="mx-3 mb-2 px-3 py-1.5 rounded-lg text-[10px] text-center"
                    style={{ background: 'rgba(108,99,255,0.06)', color: '#4a4e78', border: '1px solid rgba(108,99,255,0.12)' }}>
                    Drag onto canvas or click to add
                  </div>
                  <div className="px-2 pb-4">
                    {search.trim() ? (
                      filtered.length === 0 ? <div className="text-center py-8 text-xs" style={{ color: '#4a4e78' }}>No nodes found</div>
